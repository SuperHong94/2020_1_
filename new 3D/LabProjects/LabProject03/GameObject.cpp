...
{
	m_xmf4x4World._41 = playerPos.x;
	m_xmf4x4World._42 = playerPos.y;
	m_xmf4x4World._43 = playerPos.z;
}
CBullet::~CBullet()
{

	XMFLOAT3 dis;
	XMStoreFloat3(&dis, XMVector3Length(XMVectorSubtract(XMLoadFloat3(&nowPos), XMLoadFloat3(&startPos))));  //startPos와 nowPos사이거리
	if (dis.x > 100.0f) //거리가 100.0f보다 늘어나면 삭제
		delete this;
}
