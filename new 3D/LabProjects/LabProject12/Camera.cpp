...
void CCamera::UpdateShaderVariables(ID3D12GraphicsCommandList* pd3dCommandList)
{
	...
	//루트 파라메터 인덱스 1의
	pd3dCommandList->SetGraphicsRoot32BitConstants(1, 16, &xmf4x4View, 0); //배열 넘긴다. 
	XMFLOAT4X4 xmf4x4Projection;
	XMStoreFloat4x4(&xmf4x4Projection, XMMatrixTranspose(XMLoadFloat4x4(&m_xmf4x4Projection)));
	pd3dCommandList->SetGraphicsRoot32BitConstants(1, 16, &xmf4x4Projection, 16); //루트 파라미터 인덱스 1번 을 넘기고 16개의 32비트 값이 있고 오프셋은 16번쨰
}
...
{
	pd3dCommandList->RSSetViewports(1, &m_d3dViewport); //뷰포트 1개 셋함
	pd3dCommandList->RSSetScissorRects(1, &m_d3dScissorRect); //시저사각형 한개 넘겨줌
}
...
CThirdPersonCamera::CThirdPersonCamera(CCamera* pCamera) : CCamera(pCamera)
{
	...
	if (pCamera)
	{
		...
		그리고 로컬 x-축 벡터와 로컬 z-축 벡터의 y-좌표가 0.0f가 되도록 한다. 이것은 로컬 x-축 벡터와 로컬 z-축 벡터를 xz-평면(지면)으로 투영하는
