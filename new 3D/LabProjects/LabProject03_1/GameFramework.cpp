...
void CGameFramework::BuildFrameBuffer() //이전과 같음
{
...
void CGameFramework::ClearFrameBuffer(DWORD dwColor) //이전과 같음
{
...
void CGameFramework::PresentFrameBuffer() //이전과 같음
{
	HDC hDC = ::GetDC(m_hWnd);
	...
	nMessageID, WPARAM wParam, LPARAM lParam)
{
	switch (nMessageID)
	{
		//마우스 캡쳐를 하고 현재 마우스 위치를 가져온다. case WM_RBUTTONDOWN:
	case WM_LBUTTONDOWN:
		::SetCapture(hWnd);
...
void CGameFramework::ProcessInput()
{
	...
	if (::GetCapture() == m_hWnd)
	{
		...
		::SetCursor(NULL);
		POINT ptCursorPos;
		//현재 마우스 커서의 위치를 가져온다. ::GetCursorPos(&ptCursorPos);
