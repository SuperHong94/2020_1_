...
void CGameFramework::CreateDirect3DDevice() //디바이스는 팩토리 만들고  팩토리에서 어댑터 열거해서 만든다.
{
	HRESULT hResult;
...
#endif
	if (SUCCEEDED(hResult = ::CreateDXGIFactory2(nDXGIFactoryFlags, __uuidof(IDXGIFactory4), (void**)&m_pdxgiFactory)))
	{

#if defined(_DEBUG)
		OutputDebugString(L"팩토리생성 성공");
#endif
	}
...
void CGameFramework::CreateCommandQueueAndList()
{
	...
	d3dCommandQueueDesc.Type = D3D12_COMMAND_LIST_TYPE_DIRECT; //그림그리기 타입
	if (SUCCEEDED(m_pd3dDevice->CreateCommandQueue(&d3dCommandQueueDesc, _uuidof(ID3D12CommandQueue), (void**)&m_pd3dCommandQueue))) {
#ifdef _DEBUG
		OutputDebugString(L"커맨큐 만들기 성공!");
#endif // _DEBUG
...
void CGameFramework::WaitForGpuComplete()
{
	...
	UINT64 u = m_pd3dFence->GetCompletedValue();
	if (u < nFence)
	{
...
void CGameFramework::FrameAdvance()
{
	...
	d3dResourceBarrier.Type = D3D12_RESOURCE_BARRIER_TYPE_TRANSITION;  //전이장벽
