...
//다음 함수는 응용프로그램이실행되어 주 윈도우가 생성되면 호출된다는 것에 유의하라.
...
void CGameFramework::OnDestroy()
{
	...
	::CloseHandle(m_hFenceEvent); //전역함수호출
...
void CGameFramework::CreateSwapChain()
{
	...
	DXGI_SWAP_CHAIN_DESC1 dxgiSwapChainDesc; ::ZeroMemory(&dxgiSwapChainDesc, sizeof(DXGI_SWAP_CHAIN_DESC1));
...
{
	//모든 하드웨어 어댑터 대하여 특성레벨 12.0을 지원하는 하드웨어 디바이스를 생성한다.
	HRESULT hResult;
...
(D3D12_RESOURCE_STATE_PRESENT)에서 렌더 타겟 상태(D3D12_RESOURCE_STATE_RENDER_TARGET)로 바뀔 것이다.*/
	...
	/*스왑체인을 프리젠트한다. 프리젠트를 하면 현재 렌더 타켓(후면버퍼)의 내용이 전면 버퍼로 옮겨지고 렌더 타겟
	인덱스가 바뀔 것이다.*/
